...
*Layer 2 — live Azure running-state.* Query the deployed estate read-only, `list` and `show` only, and flag the state that no code read can see. The recurring finding classes: a Postgres server with `publicNetworkAccess = Enabled` or a `0.0.0.0` "allow all Azure" firewall rule; a single home IP allow-listed into prod; an engine past end-of-life; `md5` password encryption where SCRAM is available; a Key Vault on public network with purge protection off; a container registry with `adminUserEnabled` (a shared, non-attributable push credential); a storage account defaulting to TLS 1.0 or still allowing shared-key access; Defender left on the free tier so the databases get no anomaly detection; resources shipping zero diagnostic settings, so failed-auth events are ephemeral and go nowhere.
...
*Layer 3 — live Entra and ADO.* The identity and pipeline plane. On the tenant, read the app registrations (`az ad app list/show`): reply URLs still pointing at `localhost`, `http://`, postman, or ngrok on a real app-reg; implicit or hybrid grant issuing tokens into the URL fragment; client secrets or certs valid for more than five years; reply URLs on non-Azure hosts you have to confirm are still owned (an orphaned one is a takeover); one app-reg spanning prod and non-prod, where a non-prod compromise mints prod tokens. On the build system, read variable groups _and_ build-definition variables for values not marked secret that look like tokens by name or shape — the build-definition path is the one a variable-group-only check misses, and it is exactly where a Sonar or registry token tends to sit in cleartext.
...
*Never push to a client default, never auto-merge.* The fix belongs to the client's engineers and their process. Landing it on their default branch, or merging it because the ruleset happens to permit admin-merge, takes a decision that is theirs and skips the review that catches the fix that breaks their API or reintroduces a bug. The audit opens the PR; a human on their side merges it. Keeping strictly to that line is what keeps the audit a collaborator rather than an unrequested committer.
...
Standing methodology to judge every audit against, as of its own date: PTES for the overall shape, OWASP WSTG for web surfaces, NIST SP 800-115 for authorization and documentation discipline, CVSS 4.0 for scoring, and coordinated disclosure for how a finding travels from private to fixed.
