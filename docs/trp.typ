...
#show: skill-doc.with(
  ...
  when: "A ticket is assigned and needs delivering to a professional standard: understood against the real repo, planned, approved, implemented, verified, reviewed, and closed.",
...
The trap to know: many repos run tests as continue-on-error in CI, so a green CI badge can sit on top of a failing test. A green check is not proof. Gate on the real verdict — the actual test output — not the colour of the check. A failing test is a hard block; you do not push red. The only exception is a failure you can _prove_ is pre-existing on the trunk baseline and unrelated to your change. You prove it by stashing your diff and watching it fail without you. Scan the commit message for AI attribution before every push (the exact grep is in the cheatsheet below).

Then the change-specific battery. Exercise the _real_ surface the change lives on — if the change is about a save path, drive a real save, don't test the DOM around it. Where a flag exists, prove the change is inert with the flag off. Enumerate every entrypoint and call-site of the behaviour you changed, across the _whole_ repo — scripts, tooling, migrations, seeds, not just the `src/` directory. A grep scoped to `src/` structurally cannot see a prod-capable seed script wired as an npm task, and that is exactly where a live-data surprise hides. State the search you ran, so the scope itself is reviewable. Anything you genuinely can't prove, you report as unproven — never silently passed.
