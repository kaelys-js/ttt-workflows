...
description: Audit product COPY and UI microcopy — and, with --mode=comments, code-comment slop and test-runner names (subsumes the comment-audit skill) — across a git repo or diff, against best-practice standards as of August 2026. AST-based extraction (tree-sitter for 40+ languages plus babel/Astro/remark/yaml) covers markdown prose/headings/alt-text, JSON/YAML/TOML copy values, UI microcopy in HTML/Astro/Svelte/Vue/JSX/TSX, config/templates/data, and (comment mode) comment blocks + it/describe/test names — judged with full-file context by parallel subagents against four cited pillars (plain language, inclusive, UX microcopy, voice/grammar) or comment-quality + Rule 9. Verdicts keep/rewrite/flag/delete; SHA-guarded char-offset splicing changes only text, never structure. Use for /copy-audit, copy/content/microcopy review, readability/plain-language/inclusive pass, markdown/docs copy, and comment/test-name audit — "comment cleanup", "AI comment slop", "test name audit", "Rule 9 sweep".
