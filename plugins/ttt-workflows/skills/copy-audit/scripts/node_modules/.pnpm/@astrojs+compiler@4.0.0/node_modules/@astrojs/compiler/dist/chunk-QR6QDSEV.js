var l = () => {
		let r = new Error('not implemented');
		return ((r.code = 'ENOSYS'), r);
	},
	g = '',
	w = {
		...
		writeSync(r, n) {
			...
			let s = g.lastIndexOf(`
`);
			return (s != -1 && (console.log(g.substr(0, s)), (g = g.substr(s + 1))), n.length);
		},
		write(r, n, s, i, h, c) {
			if (s !== 0 || i !== n.length || h !== null) {
				c(l());
				return;
			}
			let d = this.writeSync(r, n);
			c(null, d);
		},
		chmod(r, n, s) {
			s(l());
...
Object.defineProperties(globalThis, {
	fs: { value: w, enumerable: !0 },
	process: { value: b, enumerable: !0 },
});
...
var _ = class {
	constructor() {
		...
		let n = (t, e) => {
			...
			s = (t) => {
				let e = this.mem.getUint32(t + 0, !0),
					o = this.mem.getInt32(t + 4, !0);
			...
			d = (t) => {
				let e = s(t + 0),
					o = s(t + 8),
					a = new Array(o);
				for (let u = 0; u < o; u++) a[u] = i(e + u * 8);
				return a;
			},
			f = (t) => {
				let e = s(t + 0),
					o = s(t + 8);
				return x.decode(new DataView(this._inst.exports.mem.buffer, e, o));
			},
		...
		this.importObject = {
			gojs: {
				...
				'runtime.wasmWrite': (t) => {
					...
					w.writeSync(e, new Uint8Array(this._inst.exports.mem.buffer, o, a));
				},
				'runtime.resetMemoryDataView': (t) => {
					((t >>>= 0), (this.mem = new DataView(this._inst.exports.mem.buffer)));
				},
				'runtime.nanotime1': (t) => {
					((t >>>= 0), n(t + 8, (m + performance.now()) * 1e6));
				},
				'runtime.walltime': (t) => {
					t >>>= 0;
					let e = new Date().getTime();
					(n(t + 8, e / 1e3), this.mem.setInt32(t + 16, (e % 1e3) * 1e6, !0));
				},
				...
				'syscall/js.valueGet': (t) => {
					t >>>= 0;
					let e = Reflect.get(i(t + 8), f(t + 16));
					((t = this._inst.exports.getsp() >>> 0), h(t + 32, e));
				},
				'syscall/js.valueSet': (t) => {
					((t >>>= 0), Reflect.set(i(t + 8), f(t + 16), i(t + 32)));
				},
				'syscall/js.valueDelete': (t) => {
					((t >>>= 0), Reflect.deleteProperty(i(t + 8), f(t + 16)));
				},
				'syscall/js.valueIndex': (t) => {
					((t >>>= 0), h(t + 24, Reflect.get(i(t + 8), s(t + 16))));
				},
				...
				'syscall/js.valueCall': (t) => {
					...
					try {
						let e = i(t + 8),
							o = Reflect.get(e, f(t + 16)),
							a = d(t + 32),
							u = Reflect.apply(o, e, a);
						((t = this._inst.exports.getsp() >>> 0), h(t + 56, u), this.mem.setUint8(t + 64, 1));
					} catch (e) {
						((t = this._inst.exports.getsp() >>> 0), h(t + 56, e), this.mem.setUint8(t + 64, 0));
					}
				},
				'syscall/js.valueInvoke': (t) => {
					...
					try {
						let e = i(t + 8),
							o = d(t + 16),
							a = Reflect.apply(e, void 0, o);
						((t = this._inst.exports.getsp() >>> 0), h(t + 40, a), this.mem.setUint8(t + 48, 1));
					} catch (e) {
						((t = this._inst.exports.getsp() >>> 0), h(t + 40, e), this.mem.setUint8(t + 48, 0));
					}
				},
				...
				'syscall/js.valueLength': (t) => {
					((t >>>= 0), n(t + 16, Number.parseInt(i(t + 8).length)));
				},
				'syscall/js.valuePrepareString': (t) => {
					t >>>= 0;
					let e = y.encode(String(i(t + 8)));
					(h(t + 16, e), n(t + 24, e.length));
				},
				...
				'syscall/js.copyBytesToGo': (t) => {
					...
					let a = o.subarray(0, e.length);
					(e.set(a), n(t + 40, a.length), this.mem.setUint8(t + 48, 1));
				},
	...
	async run(n) {
		...
		((this._inst = n),
			...
			(this._values = [Number.NaN, 0, null, !0, !1, globalThis, this]),
			(this._goRefCounts = new Array(this._values.length).fill(Number.POSITIVE_INFINITY)),
			(this._ids = new Map([
				[0, 1],
				[null, 2],
				[!0, 3],
		...
		let s = 4096,
			i = (m) => {
				let t = s,
					e = y.encode(`${m}\0`);
				return (
					new Uint8Array(this.mem.buffer, s, e.length).set(e),
					(s += e.length),
					s % 8 !== 0 && (s += 8 - (s % 8)),
					t
				);
		...
		(this.argv.forEach((m) => {
			...
			Object.keys(this.env)
				.sort()
				.forEach((m) => {
					c.push(i(`${m}=${this.env[m]}`));
				}),
			c.push(0));
		let f = s;
		(c.forEach((m) => {
			(this.mem.setUint32(s, m, !0), this.mem.setUint32(s + 4, 0, !0), (s += 8));
		}),
			this._inst.exports.run(h, f),
	...
	_makeFuncWrapper(n) {
		let s = this;
		return function () {
			let i = { id: n, this: this, args: arguments };
			return ((s._pendingEvent = i), s._resume(), i.result);
		};
	}
};
export { _ as a };
