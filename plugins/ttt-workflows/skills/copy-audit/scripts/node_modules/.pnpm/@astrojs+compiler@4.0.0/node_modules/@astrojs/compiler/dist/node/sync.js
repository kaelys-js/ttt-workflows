import { a as c } from '../chunk-W5DTLHV4.js';
import { readFileSync as p } from 'fs';
import { fileURLToPath as m } from 'url';
function i() {
	return (s || (s = f()), s);
}
var s,
	l = (e, t) => i().transform(e, t),
	w = (e, t) => i().parse(e, t),
	h = (e, t) => i().convertToTSX(e, t);
function f() {
	let e = new c(),
		t = v(m(new URL('../astro.wasm', import.meta.url)), e.importObject);
	e.run(t);
	let o = globalThis['@astrojs/compiler'];
	return {
		transform: (n, a) => {
			try {
				return o.transform(n, a || {});
			} catch (r) {
				throw ((s = void 0), r);
			}
		},
		parse: (n, a) => {
			try {
				let r = o.parse(n, a || {});
				return { ...r, ast: JSON.parse(r.ast) };
			} catch (r) {
				throw ((s = void 0), r);
			}
		},
		convertToTSX: (n, a) => {
			try {
				let r = o.convertToTSX(n, a || {});
				return { ...r, map: JSON.parse(r.map) };
			} catch (r) {
				throw ((s = void 0), r);
			}
		},
	};
}
function v(e, t) {
	let o = p(e);
	return new WebAssembly.Instance(new WebAssembly.Module(o), t);
}
export { h as convertToTSX, w as parse, f as startRunningService, l as transform };
