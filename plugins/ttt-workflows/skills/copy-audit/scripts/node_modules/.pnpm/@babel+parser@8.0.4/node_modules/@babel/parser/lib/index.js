	...
	class ESTreeParserMixin extends superClass {
		...
		isValidLVal(type, disallowCallExpression, isUnparenthesizedInAssign, binding) {
			return type === 'Property'
				? 'value'
				: super.isValidLVal(type, disallowCallExpression, isUnparenthesizedInAssign, binding);
		}
...
const supplementaryIdentifierStartCodes = [
	2368, 25, 1388, 2, 3817, 43, 20677, 24, 3, 24, 287, 4, 6146, 7, 1290, 21, 98, 114, 22734, 30, 2,
	2, 2, 1, 2, 6, 3, 4, 10, 1, 53307, 5, 5987, 11, 21763, 4297,
];
	...
	class FlowParserMixin extends superClass {
				...
				case 6: {
					...
					if (isGroupedType) {
						...
						if (
							...
							!(this.match(8) || (this.match(7) && this.lookahead().type === 15))
						) {
							this.expect(7);
							return type;
						} else {
		...
		isValidLVal(type, disallowCallExpression, isParenthesized, binding) {
			return (
				...
				super.isValidLVal(type, disallowCallExpression, isParenthesized, binding)
			);
		}
		...
		checkParams(node, allowDuplicates, isArrowFunction, strictModeChanged = true) {
			if (
				isArrowFunction &&
				this.state.noArrowParamsConversionAt.includes(this.offsetToSourcePos(node.start))
			) {
				return;
			}
		...
		parseParenAndDistinguishExpression(canStartArrow) {
			return super.parseParenAndDistinguishExpression(
				canStartArrow && !this.state.noArrowAt.includes(this.sourceToOffsetPos(this.state.start)),
			);
		}
...
class TypeScriptScopeHandler extends ScopeHandler {
	get inTSNamespace() {
		...
		return (
			scopeStack.length >= 2 &&
			scopeStack[scopeStack.length - 1].flags === 0 &&
			(scopeStack[scopeStack.length - 2].flags & 2048) > 0
		);
	}
	importsStack = [];
	createScope(flags) {
...
class Tokenizer extends CommentsParser {
	...
	raiseOverwrite(toParseError, at, details = {}) {
		const loc =
			at instanceof Position ? at : this.optionFlags & 256 ? at.loc.start : this.getLoc(at.start);
...
class LValParser extends NodeUtils {
			...
			case 'AssignmentExpression':
				if (node.operator !== '=') {
					this.raise(
						...
						this.optionFlags & 256 ? node.left.loc.end : node.left,
					);
				}
	...
	checkLVal(
		...
		disallowCallExpression ||=
			...
			(unwrappedExpression.callee.type === 'Import' || unwrappedExpression.callee.type === 'Super');
...
class ExpressionParser extends LValParser {
	...
	parseMaybeAssign(refExpressionErrors, afterLeftParse) {
		...
		if (isYield) {
			...
			const startsExpr = this.hasPlugin('v8intrinsic')
				...
				: tokenCanStartExpression(type) && !this.match(50);
	...
	parseParenAndDistinguishExpression(canStartArrow) {
		...
		while (!this.match(7)) {
			...
			if (this.match(17)) {
				...
				exprList.push(
					this.parseMaybeAssignAllowInOrVoidPattern(7, refExpressionErrors, this.parseParenItem),
				);
			}
		}
	...
	parseObjectProperty(prop, startLoc, isPattern, refExpressionErrors) {
		...
		if (this.eat(10)) {
			prop.value = isPattern
				...
				: this.parseMaybeAssignAllowInOrVoidPattern(4, refExpressionErrors);
	...
	isAmbiguousPrefixOrIdentifier() {
		...
		return (
			...
			(type === 98 && !this.state.containsEsc) ||
			type === 133 ||
			type === 52 ||
	...
	parseMaybeAssignAllowInOrVoidPattern(close, refExpressionErrors, afterLeftParse) {
		if (refExpressionErrors != null && this.match(84)) {
			...
			if (
				nextCode === 44 ||
				nextCode === (close === 1 ? 93 : close === 4 ? 125 : 41) ||
				nextCode === 61
			) {
...
class StatementParser extends ExpressionParser {
		...
		for (;;) {
			...
			if (decl.init === null && !allowMissingInitializer) {
				if (
					...
					!(isFor && (this.match(54) || this.isContextual(98)))
				) {
					...
					!(this.match(54) || this.isContextual(98))
				) {
					this.raise(Errors.DeclarationMissingInitializer, this.state.lastTokEndLoc, {
						kind,
					});
	...
	isExportDefaultSpecifier() {
		...
		if (tokenIsIdentifier(type)) {
			...
			if ((type === 126 || type === 125) && !this.state.containsEsc) {
				...
				if (
					nextChar === 123 ||
					(this.chStartsBindingIdentifier(nextChar, next) && !this.input.startsWith('from', next))
				) {
	...
	parseMaybeImportPhase(node, isExport) {
		...
		const isImportPhase = tokenIsKeywordOrIdentifier(type)
			? type !== 94 || this.lookaheadCharCode() === 102
			: type !== 8;
		if (isImportPhase) {
...
const TSErrorTemplates = {
	...
	UnexpectedTypeDeclaration: (type) =>
...
const typescript = (superClass) =>
	class TypeScriptParserMixin extends superClass {
		...
		tsIsStartOfFunctionType() {
			...
			return (
				this.match(6) && this.tsLookAhead(this.tsIsUnambiguouslyStartOfFunctionType.bind(this))
			);
		}
			...
			switch (type) {
				case 120:
					if (
						...
						(this.match(76) || tokenIsIdentifier(this.state.type))
					) {
		...
		parseExprOp(left, leftStartLoc, minPrec) {
			let isSatisfies;
			if (
				tokenOperatorPrecedence(54) > minPrec &&
				!this.hasPrecedingLineBreak() &&
				(this.isContextual(89) || (isSatisfies = this.isContextual(116)))
			) {
				const node = this.startNodeAt(leftStartLoc);
