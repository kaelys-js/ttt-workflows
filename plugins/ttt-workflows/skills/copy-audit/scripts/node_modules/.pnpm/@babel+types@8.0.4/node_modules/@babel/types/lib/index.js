...
function assertNodeOrValueType(...types) {
	function validate(node, key, val) {
		...
		throw new TypeError(
			`Property ${key} of ${node.type} expected node to be of a type ${JSON.stringify(types)} but instead got ${JSON.stringify(val?.type)}`,
		);
	}
				...
				properties: {
					key: {
						validate: allowPrivateName
							...
							: assertNodeType('Identifier', 'StringLiteral', 'NumericLiteral', 'BigIntLiteral'),
					},
				},
			},
		],
		...
		value: {
			validate: chain(
				...
				function templateElementCookedValidator(node) {
					...
					const { str, firstInvalidLoc } = readStringContents('template', raw, 0, 0, 0, {
						unterminated() {
...
function inherit(key, child, parent) {
	if (child && parent) {
		...
		child[key] = Array.from(
			childVal ? new Set([...childVal, ...parentVal].filter(Boolean)) : parentVal,
		);
	}
}
...
function getBindingIdentifiers(node, duplicates, outerOnly, newBindingsOnly) {
	...
	while (search.length) {
		...
		if (
			newBindingsOnly &&
			(isAssignmentExpression(id) || isUnaryExpression(id) || isUpdateExpression(id))
		) {
			continue;
		}
		if (isIdentifier(id)) {
...
export {
	...
	removeTypeDuplicates$1 as removeTypeDuplicates,
	restElement,
