...
var Edit = class {
	...
	editPoint(point, index) {
		...
		if (index >= this.oldEndIndex) {
			...
			newPoint.column =
				originalRow === this.oldEndPosition.row
	...
	editRange(range) {
		...
		if (range.endIndex >= this.oldEndIndex) {
			if (range.endIndex !== Number.MAX_SAFE_INTEGER) {
				...
				newRange.endPosition = {
					...
					column:
						range.endPosition.row === this.oldEndPosition.row
		...
		if (range.startIndex >= this.oldEndIndex) {
			...
			newRange.startPosition = {
				...
				column:
					range.startPosition.row === this.oldEndPosition.row
...
var LookaheadIterator = class {
	...
	get currentType() {
		...
		return (
			this.language.types[id] ?? C.UTF8ToString(C._ts_language_symbol_name(this.language[0], id))
		);
	}
...
${JSON.stringify(symbolNames, null, 2)}`);
			throw new Error(
				`Language.${sync ? 'loadSync' : 'load'} failed: no language function found in Wasm file`,
			);
		}
...
async function Module2(moduleArg = {}) {
	...
	function makeInvalidEarlyAccess(name2) {
		return () =>
			assert(false, `call to '${name2}' via reference taken before Wasm module initialization`);
	}
	...
	function isExportedByForceFilesystem(name2) {
		return (
			name2 === 'FS_createPath' ||
			name2 === 'FS_createDataFile' ||
			name2 === 'FS_createPreloadedFile' ||
			name2 === 'FS_preloadFile' ||
			name2 === 'FS_unlink' ||
			name2 === 'addRunDependency' || // The old FS has some functionality that WasmFS lacks.
			name2 === 'FS_createLazyFile' ||
			name2 === 'FS_createDevice' ||
			name2 === 'removeRunDependency'
		);
	}
	...
	function createExportWrapper(name2, nargs) {
		return (...args2) => {
			assert(
				runtimeInitialized,
				`native function \`${name2}\` called before runtime initialization`,
			);
			var f = wasmExports[name2];
			assert(f, `exported native function \`${name2}\` not found`);
			assert(
				args2.length <= nargs,
				`native function \`${name2}\` called with ${args2.length} args but expects ${nargs}`,
			);
			return f(...args2);
		};
	}
	...
	var LE_HEAP_LOAD_I64 = /* @__PURE__ */ __name(
		(byteOffset) => HEAP_DATA_VIEW.getBigInt64(byteOffset, true),
		'LE_HEAP_LOAD_I64',
	);
	...
	var LE_HEAP_LOAD_U64 = /* @__PURE__ */ __name(
		(byteOffset) => HEAP_DATA_VIEW.getBigUint64(byteOffset, true),
		'LE_HEAP_LOAD_U64',
	);
	...
	var LE_HEAP_STORE_I64 = /* @__PURE__ */ __name(
		(byteOffset, value) => HEAP_DATA_VIEW.setBigInt64(byteOffset, value, true),
		'LE_HEAP_STORE_I64',
	);
	...
	var LE_HEAP_STORE_U64 = /* @__PURE__ */ __name(
		(byteOffset, value) => HEAP_DATA_VIEW.setBigUint64(byteOffset, value, true),
		'LE_HEAP_STORE_U64',
	);
	...
	var generateTypePack = /* @__PURE__ */ __name(
		(types) =>
			uleb128EncodeWithLen(
		...
		(binary, flags, libName, localScope, handle) => {
			...
			function loadModule() {
				...
				if (handle) {
					...
					LE_HEAP_STORE_U32(
						SAFE_HEAP_INDEX(HEAPU32, (handle + 12) >> 2, 'storing') * 4,
						memoryBase,
					);
					LE_HEAP_STORE_I32(
						SAFE_HEAP_INDEX(HEAP32, (handle + 16) >> 2, 'storing') * 4,
						metadata.memorySize,
					);
					LE_HEAP_STORE_U32(SAFE_HEAP_INDEX(HEAPU32, (handle + 20) >> 2, 'storing') * 4, tableBase);
					LE_HEAP_STORE_I32(
						SAFE_HEAP_INDEX(HEAP32, (handle + 24) >> 2, 'storing') * 4,
						metadata.tableSize,
					);
				}
				...
				if (flags.loadAsync) {
					return (async () => {
						...
						if (binary instanceof WebAssembly.Module) {
							...
							({ module: binary, instance: instance2 } = await WebAssembly.instantiate(
								binary,
								info,
							));
						}
	...
	function loadDynamicLibrary(
		...
		if (dso) {
			assert(
				dso.exports !== 'loading',
				`Attempt to load '${libName2}' twice before the first load completed`,
			);
			if (!flags2.global) {
				if (localScope2) {
		...
		function loadLibData() {
			if (handle2) {
				...
				var dataSize = LE_HEAP_LOAD_U32(
					SAFE_HEAP_INDEX(HEAPU32, (handle2 + 32) >> 2, 'loading') * 4,
				);
				if (data && dataSize) {
	...
	var _ts_language_symbol_count = (Module['_ts_language_symbol_count'] = makeInvalidEarlyAccess(
		'_ts_language_symbol_count',
	));
	var _ts_language_state_count = (Module['_ts_language_state_count'] = makeInvalidEarlyAccess(
		'_ts_language_state_count',
	));
	var _ts_language_abi_version = (Module['_ts_language_abi_version'] = makeInvalidEarlyAccess(
		'_ts_language_abi_version',
	));
	var _ts_language_name = (Module['_ts_language_name'] =
		makeInvalidEarlyAccess('_ts_language_name'));
	var _ts_language_field_count = (Module['_ts_language_field_count'] = makeInvalidEarlyAccess(
		'_ts_language_field_count',
	));
	var _ts_language_next_state = (Module['_ts_language_next_state'] =
		makeInvalidEarlyAccess('_ts_language_next_state'));
	var _ts_language_symbol_name = (Module['_ts_language_symbol_name'] = makeInvalidEarlyAccess(
		'_ts_language_symbol_name',
	));
	var _ts_language_symbol_for_name = (Module['_ts_language_symbol_for_name'] =
		makeInvalidEarlyAccess('_ts_language_symbol_for_name'));
	var _strncmp = (Module['_strncmp'] = makeInvalidEarlyAccess('_strncmp'));
	var _ts_language_symbol_type = (Module['_ts_language_symbol_type'] = makeInvalidEarlyAccess(
		'_ts_language_symbol_type',
	));
	var _ts_language_field_name_for_id = (Module['_ts_language_field_name_for_id'] =
		makeInvalidEarlyAccess('_ts_language_field_name_for_id'));
	var _ts_lookahead_iterator_new = (Module['_ts_lookahead_iterator_new'] = makeInvalidEarlyAccess(
		'_ts_lookahead_iterator_new',
	));
	var _ts_lookahead_iterator_delete = (Module['_ts_lookahead_iterator_delete'] =
		makeInvalidEarlyAccess('_ts_lookahead_iterator_delete'));
	var _ts_lookahead_iterator_reset_state = (Module['_ts_lookahead_iterator_reset_state'] =
		makeInvalidEarlyAccess('_ts_lookahead_iterator_reset_state'));
	var _ts_lookahead_iterator_reset = (Module['_ts_lookahead_iterator_reset'] =
		makeInvalidEarlyAccess('_ts_lookahead_iterator_reset'));
	var _ts_lookahead_iterator_next = (Module['_ts_lookahead_iterator_next'] = makeInvalidEarlyAccess(
		'_ts_lookahead_iterator_next',
	));
	var _ts_lookahead_iterator_current_symbol = (Module['_ts_lookahead_iterator_current_symbol'] =
		makeInvalidEarlyAccess('_ts_lookahead_iterator_current_symbol'));
	...
	var _ts_parser_delete = (Module['_ts_parser_delete'] =
		makeInvalidEarlyAccess('_ts_parser_delete'));
	var _ts_parser_set_language = (Module['_ts_parser_set_language'] =
		makeInvalidEarlyAccess('_ts_parser_set_language'));
	...
	var _ts_parser_set_included_ranges = (Module['_ts_parser_set_included_ranges'] =
		makeInvalidEarlyAccess('_ts_parser_set_included_ranges'));
	...
	var _ts_query_pattern_count = (Module['_ts_query_pattern_count'] =
		makeInvalidEarlyAccess('_ts_query_pattern_count'));
	var _ts_query_capture_count = (Module['_ts_query_capture_count'] =
		makeInvalidEarlyAccess('_ts_query_capture_count'));
	var _ts_query_string_count = (Module['_ts_query_string_count'] =
		makeInvalidEarlyAccess('_ts_query_string_count'));
	var _ts_query_capture_name_for_id = (Module['_ts_query_capture_name_for_id'] =
		makeInvalidEarlyAccess('_ts_query_capture_name_for_id'));
	var _ts_query_capture_quantifier_for_id = (Module['_ts_query_capture_quantifier_for_id'] =
		makeInvalidEarlyAccess('_ts_query_capture_quantifier_for_id'));
	var _ts_query_string_value_for_id = (Module['_ts_query_string_value_for_id'] =
		makeInvalidEarlyAccess('_ts_query_string_value_for_id'));
	var _ts_query_predicates_for_pattern = (Module['_ts_query_predicates_for_pattern'] =
		makeInvalidEarlyAccess('_ts_query_predicates_for_pattern'));
	var _ts_query_start_byte_for_pattern = (Module['_ts_query_start_byte_for_pattern'] =
		makeInvalidEarlyAccess('_ts_query_start_byte_for_pattern'));
	var _ts_query_end_byte_for_pattern = (Module['_ts_query_end_byte_for_pattern'] =
		makeInvalidEarlyAccess('_ts_query_end_byte_for_pattern'));
	var _ts_query_is_pattern_rooted = (Module['_ts_query_is_pattern_rooted'] = makeInvalidEarlyAccess(
		'_ts_query_is_pattern_rooted',
	));
	var _ts_query_is_pattern_non_local = (Module['_ts_query_is_pattern_non_local'] =
		makeInvalidEarlyAccess('_ts_query_is_pattern_non_local'));
	var _ts_query_is_pattern_guaranteed_at_step = (Module['_ts_query_is_pattern_guaranteed_at_step'] =
		makeInvalidEarlyAccess('_ts_query_is_pattern_guaranteed_at_step'));
	var _ts_query_disable_capture = (Module['_ts_query_disable_capture'] = makeInvalidEarlyAccess(
		'_ts_query_disable_capture',
	));
	var _ts_query_disable_pattern = (Module['_ts_query_disable_pattern'] = makeInvalidEarlyAccess(
	...
	function assignWasmExports(wasmExports2) {
		...
		Module['_ts_language_symbol_count'] = _ts_language_symbol_count = createExportWrapper(
			'ts_language_symbol_count',
			1,
		);
		Module['_ts_language_state_count'] = _ts_language_state_count = createExportWrapper(
			'ts_language_state_count',
			1,
		);
		Module['_ts_language_abi_version'] = _ts_language_abi_version = createExportWrapper(
			'ts_language_abi_version',
			1,
		);
		Module['_ts_language_name'] = _ts_language_name = createExportWrapper('ts_language_name', 1);
		Module['_ts_language_field_count'] = _ts_language_field_count = createExportWrapper(
			'ts_language_field_count',
			1,
		);
		Module['_ts_language_next_state'] = _ts_language_next_state = createExportWrapper(
			'ts_language_next_state',
			3,
		);
		Module['_ts_language_symbol_name'] = _ts_language_symbol_name = createExportWrapper(
			'ts_language_symbol_name',
			2,
		);
		Module['_ts_language_symbol_for_name'] = _ts_language_symbol_for_name = createExportWrapper(
			'ts_language_symbol_for_name',
			4,
		);
		Module['_strncmp'] = _strncmp = createExportWrapper('strncmp', 3);
		Module['_ts_language_symbol_type'] = _ts_language_symbol_type = createExportWrapper(
			'ts_language_symbol_type',
			2,
		);
		Module['_ts_language_field_name_for_id'] = _ts_language_field_name_for_id = createExportWrapper(
			'ts_language_field_name_for_id',
			2,
		);
		Module['_ts_lookahead_iterator_new'] = _ts_lookahead_iterator_new = createExportWrapper(
			'ts_lookahead_iterator_new',
			2,
		);
		Module['_ts_lookahead_iterator_delete'] = _ts_lookahead_iterator_delete = createExportWrapper(
			'ts_lookahead_iterator_delete',
			1,
		);
		Module['_ts_lookahead_iterator_reset_state'] = _ts_lookahead_iterator_reset_state =
			createExportWrapper('ts_lookahead_iterator_reset_state', 2);
		Module['_ts_lookahead_iterator_reset'] = _ts_lookahead_iterator_reset = createExportWrapper(
			'ts_lookahead_iterator_reset',
			3,
		);
		Module['_ts_lookahead_iterator_next'] = _ts_lookahead_iterator_next = createExportWrapper(
			'ts_lookahead_iterator_next',
			1,
		);
		Module['_ts_lookahead_iterator_current_symbol'] = _ts_lookahead_iterator_current_symbol =
			createExportWrapper('ts_lookahead_iterator_current_symbol', 1);
		...
		Module['_ts_parser_set_language'] = _ts_parser_set_language = createExportWrapper(
			'ts_parser_set_language',
			2,
		);
		Module['_ts_parser_reset'] = _ts_parser_reset = createExportWrapper('ts_parser_reset', 1);
		Module['_ts_parser_set_included_ranges'] = _ts_parser_set_included_ranges = createExportWrapper(
		...
		Module['_ts_query_pattern_count'] = _ts_query_pattern_count = createExportWrapper(
			'ts_query_pattern_count',
			1,
		);
		Module['_ts_query_capture_count'] = _ts_query_capture_count = createExportWrapper(
			'ts_query_capture_count',
			1,
		);
		Module['_ts_query_string_count'] = _ts_query_string_count = createExportWrapper(
			'ts_query_string_count',
			1,
		);
		Module['_ts_query_capture_name_for_id'] = _ts_query_capture_name_for_id = createExportWrapper(
			'ts_query_capture_name_for_id',
			3,
		);
		Module['_ts_query_capture_quantifier_for_id'] = _ts_query_capture_quantifier_for_id =
			createExportWrapper('ts_query_capture_quantifier_for_id', 3);
		Module['_ts_query_string_value_for_id'] = _ts_query_string_value_for_id = createExportWrapper(
			'ts_query_string_value_for_id',
			3,
		);
		Module['_ts_query_predicates_for_pattern'] = _ts_query_predicates_for_pattern =
			createExportWrapper('ts_query_predicates_for_pattern', 3);
		Module['_ts_query_start_byte_for_pattern'] = _ts_query_start_byte_for_pattern =
			createExportWrapper('ts_query_start_byte_for_pattern', 2);
		Module['_ts_query_end_byte_for_pattern'] = _ts_query_end_byte_for_pattern = createExportWrapper(
			'ts_query_end_byte_for_pattern',
			2,
		);
		Module['_ts_query_is_pattern_rooted'] = _ts_query_is_pattern_rooted = createExportWrapper(
			'ts_query_is_pattern_rooted',
			2,
		);
		Module['_ts_query_is_pattern_non_local'] = _ts_query_is_pattern_non_local = createExportWrapper(
			'ts_query_is_pattern_non_local',
			2,
		);
		Module['_ts_query_is_pattern_guaranteed_at_step'] = _ts_query_is_pattern_guaranteed_at_step =
			createExportWrapper('ts_query_is_pattern_guaranteed_at_step', 2);
		Module['_ts_query_disable_capture'] = _ts_query_disable_capture = createExportWrapper(
			'ts_query_disable_capture',
			3,
		);
		Module['_ts_query_disable_pattern'] = _ts_query_disable_pattern = createExportWrapper(
		...
		Module['_ts_parser_new_wasm'] = _ts_parser_new_wasm = createExportWrapper(
			'ts_parser_new_wasm',
			0,
		);
		Module['_ts_parser_enable_logger_wasm'] = _ts_parser_enable_logger_wasm = createExportWrapper(
			'ts_parser_enable_logger_wasm',
			2,
		);
		Module['_ts_parser_parse_wasm'] = _ts_parser_parse_wasm = createExportWrapper(
			'ts_parser_parse_wasm',
			5,
		);
		Module['_ts_parser_included_ranges_wasm'] = _ts_parser_included_ranges_wasm =
			createExportWrapper('ts_parser_included_ranges_wasm', 1);
		Module['_ts_language_type_is_named_wasm'] = _ts_language_type_is_named_wasm =
			createExportWrapper('ts_language_type_is_named_wasm', 2);
		Module['_ts_language_type_is_visible_wasm'] = _ts_language_type_is_visible_wasm =
			createExportWrapper('ts_language_type_is_visible_wasm', 2);
		Module['_ts_language_metadata_wasm'] = _ts_language_metadata_wasm = createExportWrapper(
			'ts_language_metadata_wasm',
			1,
		);
		Module['_ts_language_supertypes_wasm'] = _ts_language_supertypes_wasm = createExportWrapper(
			'ts_language_supertypes_wasm',
			1,
		);
		Module['_ts_language_subtypes_wasm'] = _ts_language_subtypes_wasm = createExportWrapper(
			'ts_language_subtypes_wasm',
			2,
		);
		Module['_ts_tree_root_node_wasm'] = _ts_tree_root_node_wasm = createExportWrapper(
			'ts_tree_root_node_wasm',
			1,
		);
		Module['_ts_tree_root_node_with_offset_wasm'] = _ts_tree_root_node_with_offset_wasm =
			createExportWrapper('ts_tree_root_node_with_offset_wasm', 1);
		Module['_ts_tree_edit_wasm'] = _ts_tree_edit_wasm = createExportWrapper('ts_tree_edit_wasm', 1);
		Module['_ts_tree_included_ranges_wasm'] = _ts_tree_included_ranges_wasm = createExportWrapper(
			'ts_tree_included_ranges_wasm',
			1,
		);
		Module['_ts_tree_get_changed_ranges_wasm'] = _ts_tree_get_changed_ranges_wasm =
			createExportWrapper('ts_tree_get_changed_ranges_wasm', 2);
		Module['_ts_tree_cursor_new_wasm'] = _ts_tree_cursor_new_wasm = createExportWrapper(
			'ts_tree_cursor_new_wasm',
			1,
		);
		Module['_ts_tree_cursor_copy_wasm'] = _ts_tree_cursor_copy_wasm = createExportWrapper(
			'ts_tree_cursor_copy_wasm',
			1,
		);
		Module['_ts_tree_cursor_delete_wasm'] = _ts_tree_cursor_delete_wasm = createExportWrapper(
			'ts_tree_cursor_delete_wasm',
			1,
		);
		Module['_ts_tree_cursor_reset_wasm'] = _ts_tree_cursor_reset_wasm = createExportWrapper(
			'ts_tree_cursor_reset_wasm',
			1,
		);
		Module['_ts_tree_cursor_reset_to_wasm'] = _ts_tree_cursor_reset_to_wasm = createExportWrapper(
			'ts_tree_cursor_reset_to_wasm',
			2,
		);
		Module['_ts_tree_cursor_goto_first_child_wasm'] = _ts_tree_cursor_goto_first_child_wasm =
			createExportWrapper('ts_tree_cursor_goto_first_child_wasm', 1);
		Module['_ts_tree_cursor_goto_last_child_wasm'] = _ts_tree_cursor_goto_last_child_wasm =
			createExportWrapper('ts_tree_cursor_goto_last_child_wasm', 1);
		Module['_ts_tree_cursor_goto_first_child_for_index_wasm'] =
			_ts_tree_cursor_goto_first_child_for_index_wasm = createExportWrapper(
				'ts_tree_cursor_goto_first_child_for_index_wasm',
				1,
			);
		Module['_ts_tree_cursor_goto_first_child_for_position_wasm'] =
			_ts_tree_cursor_goto_first_child_for_position_wasm = createExportWrapper(
		...
		Module['_ts_tree_cursor_goto_next_sibling_wasm'] = _ts_tree_cursor_goto_next_sibling_wasm =
			createExportWrapper('ts_tree_cursor_goto_next_sibling_wasm', 1);
		Module['_ts_tree_cursor_goto_previous_sibling_wasm'] =
			_ts_tree_cursor_goto_previous_sibling_wasm = createExportWrapper(
		...
		Module['_ts_tree_cursor_goto_descendant_wasm'] = _ts_tree_cursor_goto_descendant_wasm =
			createExportWrapper('ts_tree_cursor_goto_descendant_wasm', 2);
		Module['_ts_tree_cursor_goto_parent_wasm'] = _ts_tree_cursor_goto_parent_wasm =
			createExportWrapper('ts_tree_cursor_goto_parent_wasm', 1);
		Module['_ts_tree_cursor_current_node_type_id_wasm'] =
			_ts_tree_cursor_current_node_type_id_wasm = createExportWrapper(
				'ts_tree_cursor_current_node_type_id_wasm',
				1,
			);
		Module['_ts_tree_cursor_current_node_state_id_wasm'] =
			_ts_tree_cursor_current_node_state_id_wasm = createExportWrapper(
				'ts_tree_cursor_current_node_state_id_wasm',
				1,
			);
		Module['_ts_tree_cursor_current_node_is_named_wasm'] =
			_ts_tree_cursor_current_node_is_named_wasm = createExportWrapper(
				'ts_tree_cursor_current_node_is_named_wasm',
				1,
			);
		Module['_ts_tree_cursor_current_node_is_missing_wasm'] =
			_ts_tree_cursor_current_node_is_missing_wasm = createExportWrapper(
		...
		Module['_ts_tree_cursor_current_node_id_wasm'] = _ts_tree_cursor_current_node_id_wasm =
			createExportWrapper('ts_tree_cursor_current_node_id_wasm', 1);
		Module['_ts_tree_cursor_start_position_wasm'] = _ts_tree_cursor_start_position_wasm =
			createExportWrapper('ts_tree_cursor_start_position_wasm', 1);
		Module['_ts_tree_cursor_end_position_wasm'] = _ts_tree_cursor_end_position_wasm =
			createExportWrapper('ts_tree_cursor_end_position_wasm', 1);
		Module['_ts_tree_cursor_start_index_wasm'] = _ts_tree_cursor_start_index_wasm =
			createExportWrapper('ts_tree_cursor_start_index_wasm', 1);
		Module['_ts_tree_cursor_end_index_wasm'] = _ts_tree_cursor_end_index_wasm = createExportWrapper(
			'ts_tree_cursor_end_index_wasm',
			1,
		);
		Module['_ts_tree_cursor_current_field_id_wasm'] = _ts_tree_cursor_current_field_id_wasm =
			createExportWrapper('ts_tree_cursor_current_field_id_wasm', 1);
		Module['_ts_tree_cursor_current_depth_wasm'] = _ts_tree_cursor_current_depth_wasm =
			createExportWrapper('ts_tree_cursor_current_depth_wasm', 1);
		Module['_ts_tree_cursor_current_descendant_index_wasm'] =
			_ts_tree_cursor_current_descendant_index_wasm = createExportWrapper(
				'ts_tree_cursor_current_descendant_index_wasm',
				1,
			);
		Module['_ts_tree_cursor_current_node_wasm'] = _ts_tree_cursor_current_node_wasm =
			createExportWrapper('ts_tree_cursor_current_node_wasm', 1);
		Module['_ts_node_symbol_wasm'] = _ts_node_symbol_wasm = createExportWrapper(
			'ts_node_symbol_wasm',
			1,
		);
		Module['_ts_node_field_name_for_child_wasm'] = _ts_node_field_name_for_child_wasm =
			createExportWrapper('ts_node_field_name_for_child_wasm', 2);
		Module['_ts_node_field_name_for_named_child_wasm'] = _ts_node_field_name_for_named_child_wasm =
			createExportWrapper('ts_node_field_name_for_named_child_wasm', 2);
		Module['_ts_node_children_by_field_id_wasm'] = _ts_node_children_by_field_id_wasm =
			createExportWrapper('ts_node_children_by_field_id_wasm', 2);
		Module['_ts_node_first_child_for_byte_wasm'] = _ts_node_first_child_for_byte_wasm =
			createExportWrapper('ts_node_first_child_for_byte_wasm', 1);
		Module['_ts_node_first_named_child_for_byte_wasm'] = _ts_node_first_named_child_for_byte_wasm =
			createExportWrapper('ts_node_first_named_child_for_byte_wasm', 1);
		Module['_ts_node_grammar_symbol_wasm'] = _ts_node_grammar_symbol_wasm = createExportWrapper(
			'ts_node_grammar_symbol_wasm',
			1,
		);
		Module['_ts_node_child_count_wasm'] = _ts_node_child_count_wasm = createExportWrapper(
			'ts_node_child_count_wasm',
			1,
		);
		Module['_ts_node_named_child_count_wasm'] = _ts_node_named_child_count_wasm =
			createExportWrapper('ts_node_named_child_count_wasm', 1);
		Module['_ts_node_child_wasm'] = _ts_node_child_wasm = createExportWrapper(
			'ts_node_child_wasm',
			2,
		);
		Module['_ts_node_named_child_wasm'] = _ts_node_named_child_wasm = createExportWrapper(
			'ts_node_named_child_wasm',
			2,
		);
		Module['_ts_node_child_by_field_id_wasm'] = _ts_node_child_by_field_id_wasm =
			createExportWrapper('ts_node_child_by_field_id_wasm', 2);
		Module['_ts_node_next_sibling_wasm'] = _ts_node_next_sibling_wasm = createExportWrapper(
			'ts_node_next_sibling_wasm',
			1,
		);
		Module['_ts_node_prev_sibling_wasm'] = _ts_node_prev_sibling_wasm = createExportWrapper(
			'ts_node_prev_sibling_wasm',
			1,
		);
		Module['_ts_node_next_named_sibling_wasm'] = _ts_node_next_named_sibling_wasm =
			createExportWrapper('ts_node_next_named_sibling_wasm', 1);
		Module['_ts_node_prev_named_sibling_wasm'] = _ts_node_prev_named_sibling_wasm =
			createExportWrapper('ts_node_prev_named_sibling_wasm', 1);
		Module['_ts_node_descendant_count_wasm'] = _ts_node_descendant_count_wasm = createExportWrapper(
			'ts_node_descendant_count_wasm',
			1,
		);
		Module['_ts_node_parent_wasm'] = _ts_node_parent_wasm = createExportWrapper(
			'ts_node_parent_wasm',
			1,
		);
		Module['_ts_node_child_with_descendant_wasm'] = _ts_node_child_with_descendant_wasm =
			createExportWrapper('ts_node_child_with_descendant_wasm', 1);
		Module['_ts_node_descendant_for_index_wasm'] = _ts_node_descendant_for_index_wasm =
			createExportWrapper('ts_node_descendant_for_index_wasm', 1);
		Module['_ts_node_named_descendant_for_index_wasm'] = _ts_node_named_descendant_for_index_wasm =
			createExportWrapper('ts_node_named_descendant_for_index_wasm', 1);
		Module['_ts_node_descendant_for_position_wasm'] = _ts_node_descendant_for_position_wasm =
			createExportWrapper('ts_node_descendant_for_position_wasm', 1);
		Module['_ts_node_named_descendant_for_position_wasm'] =
			_ts_node_named_descendant_for_position_wasm = createExportWrapper(
				'ts_node_named_descendant_for_position_wasm',
				1,
			);
		Module['_ts_node_start_point_wasm'] = _ts_node_start_point_wasm = createExportWrapper(
			'ts_node_start_point_wasm',
			1,
		);
		Module['_ts_node_end_point_wasm'] = _ts_node_end_point_wasm = createExportWrapper(
			'ts_node_end_point_wasm',
			1,
		);
		Module['_ts_node_start_index_wasm'] = _ts_node_start_index_wasm = createExportWrapper(
			'ts_node_start_index_wasm',
			1,
		);
		Module['_ts_node_end_index_wasm'] = _ts_node_end_index_wasm = createExportWrapper(
			'ts_node_end_index_wasm',
			1,
		);
		Module['_ts_node_to_string_wasm'] = _ts_node_to_string_wasm = createExportWrapper(
			'ts_node_to_string_wasm',
			1,
		);
		Module['_ts_node_children_wasm'] = _ts_node_children_wasm = createExportWrapper(
			'ts_node_children_wasm',
			1,
		);
		Module['_ts_node_named_children_wasm'] = _ts_node_named_children_wasm = createExportWrapper(
			'ts_node_named_children_wasm',
			1,
		);
		Module['_ts_node_descendants_of_type_wasm'] = _ts_node_descendants_of_type_wasm =
			createExportWrapper('ts_node_descendants_of_type_wasm', 7);
		Module['_ts_node_is_named_wasm'] = _ts_node_is_named_wasm = createExportWrapper(
			'ts_node_is_named_wasm',
			1,
		);
		Module['_ts_node_has_changes_wasm'] = _ts_node_has_changes_wasm = createExportWrapper(
			'ts_node_has_changes_wasm',
			1,
		);
		Module['_ts_node_has_error_wasm'] = _ts_node_has_error_wasm = createExportWrapper(
			'ts_node_has_error_wasm',
			1,
		);
		Module['_ts_node_is_error_wasm'] = _ts_node_is_error_wasm = createExportWrapper(
			'ts_node_is_error_wasm',
			1,
		);
		Module['_ts_node_is_missing_wasm'] = _ts_node_is_missing_wasm = createExportWrapper(
			'ts_node_is_missing_wasm',
			1,
		);
		Module['_ts_node_is_extra_wasm'] = _ts_node_is_extra_wasm = createExportWrapper(
			'ts_node_is_extra_wasm',
			1,
		);
		Module['_ts_node_parse_state_wasm'] = _ts_node_parse_state_wasm = createExportWrapper(
			'ts_node_parse_state_wasm',
			1,
		);
		Module['_ts_node_next_parse_state_wasm'] = _ts_node_next_parse_state_wasm = createExportWrapper(
			'ts_node_next_parse_state_wasm',
			1,
		);
		Module['_ts_query_matches_wasm'] = _ts_query_matches_wasm = createExportWrapper(
			'ts_query_matches_wasm',
			16,
		);
		Module['_ts_query_captures_wasm'] = _ts_query_captures_wasm = createExportWrapper(
	...
	function callMain(args2 = []) {
		...
		args2.forEach((arg) => {
			LE_HEAP_STORE_U32(
				SAFE_HEAP_INDEX(HEAPU32, argv_ptr >> 2, 'storing') * 4,
...
var Query = class {
	...
	matches(node, options = {}) {
		...
		if (
			endContainingPosition !== ZERO_POINT &&
