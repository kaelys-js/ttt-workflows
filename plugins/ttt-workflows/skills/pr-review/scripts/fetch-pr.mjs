...
if (typeof fetch !== "function") { console.error("fetch-pr: Node 18+ required (global fetch missing)"); process.exit(1); }
...
function sh(cmd, args, opts = {}) {
  try {
    ...
    throw new Error(`${cmd} ${args.join(" ")} failed: ${out.trim() || e.message}`);
  }
}
...
function parseUrl(raw) {
  let u;
  try { u = new URL(raw.trim()); } catch { die(`not a URL: ${raw}`); }
  ...
  if (host === "github.com" || host.endsWith(".github.com")) {
    ...
    if (i < 2 || !/^\d+$/.test(parts[i + 1] || "")) die(`unrecognized GitHub PR URL (no numeric PR id): ${raw}`);
  ...
  if (isAdo) {
    ...
    let org, rest;
    if (host === "dev.azure.com") { org = parts[0]; rest = parts.slice(1); }
    else { org = host.split(".")[0]; rest = parts; }
    const i = rest.findIndex((p) => /^pullrequest$/i.test(p));
    if (i < 0 || !/^\d+$/.test(rest[i + 1] || "")) die(`unrecognized Azure DevOps PR URL (no numeric PR id): ${raw}`);
    const base = host === "dev.azure.com" ? `https://dev.azure.com/${org}` : `https://${org}.visualstudio.com`;
...
function fetchGitHub({ owner, repo, number }) {
  const R = `${owner}/${repo}`;
  const meta = JSON.parse(sh("gh", [
    "pr", "view", number, "-R", R, "--json",
  ...
  try {
    ...
    const g = JSON.parse(sh("gh", ["api", "graphql", "-f", `query=${q}`, "-F", `owner=${owner}`, "-F", `repo=${repo}`, "-F", `n=${number}`]));
    threads = (g.data.repository.pullRequest.reviewThreads.nodes || []).map((t) => {
      const c = t.comments.nodes[0] || {};
      return { path: c.path || null, line: c.line ?? null, author: c.author?.login || null, body: (c.body || "").slice(0, 500), isResolved: !!t.isResolved, isOutdated: !!t.isOutdated };
    });
  } catch (e) { console.error(`fetch-pr: warning — could not read review threads: ${e.message}`); }

  return {
    platform: "github", url: meta.url, owner, repo, number: Number(number),
    title: meta.title, body: meta.body || "", author: meta.author?.login || null,
    state: meta.state, isDraft: !!meta.isDraft, baseRef: meta.baseRefName, headRef: meta.headRefName, headSha: meta.headRefOid,
    ...
    commits: (meta.commits || []).map((c) => ({ sha: (c.oid || "").slice(0, 12), subject: (c.messageHeadline || "") })),
    diff, threads,
  };
}
...
function adoToken() {
  const t = sh("az", ["account", "get-access-token", "--resource", ADO_RESOURCE, "--query", "accessToken", "-o", "tsv"]).trim();
  if (!t) die("empty Azure DevOps access token — run `az login` for the correct tenant first");
  return t;
}

async function adoGet(url, token, asText = false) {
  const res = await fetch(url, { headers: { Authorization: `Bearer ${token}`, Accept: asText ? "text/plain" : "application/json" }, signal: AbortSignal.timeout(HTTP_TIMEOUT_MS) });
  const txt = await res.text();
  if (txt.startsWith("<!DOCTYPE html") || txt.startsWith("<html"))
    throw new Error(`ADO returned HTML (auth/tenant problem) for ${url}`);
  if (!res.ok) throw new Error(`ADO GET ${res.status} ${url}: ${txt.slice(0, 300)}`);
...
function gitDiffFile(path, oldText, newText, tmp) {
  ...
  let out = "";
  try {
    out = execFileSync("git", ["diff", "--no-index", "--no-color", "--", "a", "b"], { cwd: tmp, encoding: "utf8", maxBuffer: 64 * 1024 * 1024 });
  } catch (e) { out = e.stdout || ""; } // git diff exits 1 when files differ — expected
  return out
...
async function fetchAdo({ org, base, number }) {
  ...
  const iters = (await adoGet(`${apiRepo}/pullRequests/${number}/iterations?api-version=7.1`, token)).value || [];
  const lastIter = iters.length ? iters[iters.length - 1].id : 1;
  const changes = (await adoGet(`${apiRepo}/pullRequests/${number}/iterations/${lastIter}/changes?$top=2000&api-version=7.1`, token)).changeEntries || [];
  if (changes.length >= 2000) console.error("fetch-pr: warning — change list hit the 2000-entry cap; file list may be incomplete");

  const tmp = mkdtempSync(join(tmpdir(), "pr-review-"));
  process.on("exit", () => { try { rmSync(tmp, { recursive: true, force: true }); } catch { /* best effort */ } });
  const files = []; let diff = "";
  async function blob(path, commit) {
    ...
    try { return (await adoGet(url, token)).content ?? ""; }
    catch (e1) {
      try { return (await adoGet(url, token)).content ?? ""; }
      catch (e2) { die(`could not fetch blob ${path}@${commit.slice(0, 8)} after retry: ${e2.message} — refusing to fabricate a diff`); }
    }
  }
  for (const ch of changes) {
    const item = ch.item || {};
    if (item.isFolder || item.gitObjectType === "tree" || !item.path) continue;
    const ct = (ch.changeType || "").toLowerCase();
    const path = item.path.replace(/^\//, "");
    const isAdd = ct.includes("add"), isDel = ct.includes("delete");
    ...
    if (oldText !== newText) diff += gitDiffFile(path, oldText, newText, tmp) + "\n";
  }

  const threads = ((await adoGet(`${apiRepo}/pullRequests/${number}/threads?api-version=7.1`, token)).value || [])
    ...
    .map((t) => {
      ...
      return { path: ctx.filePath ? ctx.filePath.replace(/^\//, "") : null, line: ctx.rightFileStart?.line ?? null, author: c.author?.displayName || null, body: (c.content || "").slice(0, 500), isResolved: ["fixed", "closed", "wontfix"].includes((t.status || "").toLowerCase()) };
    });

  return {
    platform: "ado", url: `${base}/${P}/_git/${encodeURIComponent(repoName)}/pullrequest/${number}`,
    org, project, repo: repoName, number: Number(number),
    title: pr.title, body: pr.description || "", author: pr.createdBy?.displayName || null,
    state: pr.status, isDraft: !!pr.isDraft, baseRef: (pr.targetRefName || "").replace("refs/heads/", ""), headRef: (pr.sourceRefName || "").replace("refs/heads/", ""), headSha: sourceCommit || "",
    files, commits: [], diff, threads,
  };
}
...
const CLICKUP_TOKEN_FILE = process.env.CLICKUP_TOKEN_FILE
  || `${process.env.HOME}/.config/ttt/clickup.token`;
...
function clickupToken() {
  ...
  if (existsSync(CLICKUP_TOKEN_FILE)) return readFileSync(CLICKUP_TOKEN_FILE, "utf8").trim();
  return null;
}
...
function findTicketRefs(pr) {
  ...
  for (const m of hay.matchAll(/\b([A-Z][A-Z0-9_]{1,15}-\d{1,6})\b/g)) {
    ...
    if (!DENY.has(prefix)) refs.push({ id: m[1], custom: true });
  }
  // dedupe, keep order
  const seen = new Set();
...
async function fetchTicket(pr) {
  ...
  if (!pr.ticketRefs.length) return;
  const token = clickupToken();
  if (!token) { console.error("fetch-pr: note — ticket refs found but no ClickUp token; skipping ticket fetch"); return; }
  const seenIds = new Set();
  for (const ref of findTicketRefs(pr)) {
    const q = ref.custom ? `?custom_task_ids=true&team_id=${CLICKUP_TEAM_ID}` : "";
    try {
      const res = await fetch(`https://api.clickup.com/api/v2/task/${encodeURIComponent(ref.id)}${q}`, { headers: { Authorization: token }, signal: AbortSignal.timeout(HTTP_TIMEOUT_MS) });
      if (!res.ok) continue;
      const t = await res.json();
      if (seenIds.has(t.id)) continue; // same task referenced by URL id + custom id
      seenIds.add(t.id);
      const ticket = {
        id: t.id, custom_id: t.custom_id || null, name: t.name,
        status: t.status?.status || null, url: t.url,
        description: (t.description || "").slice(0, 6000),
      };
      pr.tickets.push(ticket);
      if (!pr.ticket) pr.ticket = ticket;
    } catch { /* non-fatal; try next ref */ }
  }
  if (!pr.ticket) console.error(`fetch-pr: note — ticket ref(s) ${pr.ticketRefs.join(", ")} did not resolve on ClickUp`);
}
...
if (!url) die("usage: node fetch-pr.mjs <PR_URL> [--out <path>]");
...
if (typeof pr.additions !== "number") {
  ...
  pr.additions = add; pr.deletions = del;
}
pr.fetchedAt = new Date().toISOString();
pr.diffBytes = pr.diff.length;
writeFileSync(out, JSON.stringify(pr, null, 2));
const ticketNote = pr.ticket ? ` · ticket ${pr.ticket.custom_id || pr.ticket.id} (${pr.ticket.status})` : (pr.ticketRefs?.length ? ` · ticket refs unresolved: ${pr.ticketRefs.join(",")}` : "");
console.log(`wrote ${out} — ${pr.platform} ${pr.owner || pr.org}/${pr.repo}#${pr.number} "${pr.title}" · ${pr.files.length} files · ${pr.diffBytes} diff bytes · ${pr.threads.length} threads${ticketNote}`);
