...
const rows = [];
let missing = 0;
const need = (ok, label, detail, how) => {
	rows.push({ ok, label, detail, how });
	if (!ok) {
		missing++;
	}
};
...
if (platform === 'github') {
	need(ghOk, 'gh (GitHub auth)', 'fetch the PR diff + threads', 'run:  gh auth login');
} else if (platform === 'ado') {
	need(
		azOk,
		'az (Azure DevOps login)',
		'mint the ADO bearer',
		'run:  az login --tenant <your-tenant>   (or set AZURE_CONFIG_DIR to a logged-in context)',
	);
} else {
	need(
		ghOk || azOk,
		'gh OR az',
		'GitHub PRs need gh; Azure DevOps PRs need az',
		'gh auth login   (GitHub)   |   az login --tenant <tenant>   (Azure DevOps)',
	);
	if (ghOk) {
		rows.push({ ok: true, label: 'gh (GitHub)', detail: 'authenticated' });
	}
	if (azOk) {
		rows.push({ ok: true, label: 'az (Azure DevOps)', detail: 'authenticated' });
	}
}
...
info(
	...
	'ClickUp token (optional)',
	`only for ticket-linked PRs — read from ${tokenFile}`,
...
for (const r of rows) {
	console.log(
		`  ${r.ok ? '✓' : r.optional ? '–' : '✗'} ${r.label}${r.detail ? `  (${r.detail})` : ''}`,
	);
	if (!r.ok && r.how) {
		console.log(`      → ${r.how}`);
	}
...
console.log();
if (missing) {
	console.error(`Missing ${missing} required item(s). Provide them, then re-run.`);
	process.exit(1);
}
