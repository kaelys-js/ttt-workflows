...
if (!path) {
	die('usage: node render-review.mjs <findings.json> [--platform github|ado] [--pr pr.json]');
}
const pIdx = args.indexOf('--platform');
let platform = pIdx >= 0 ? args[pIdx + 1] : process.env.PR_PLATFORM || 'github';
if (!['github', 'ado'].includes(platform)) {
	die(`--platform must be github or ado (got '${platform}')`);
}
...
if (prIdx >= 0) {
	try {
		prMeta = JSON.parse(readFileSync(args[prIdx + 1], 'utf8'));
	} catch (error) {
		die(`cannot read --pr file: ${error.message}`);
	}
	if (prMeta.platform && pIdx < 0) {
		platform = prMeta.platform;
	}
}
...
for (const f of findings) {
	...
	if (!['blocking', 'non-blocking'].includes(f.severity)) {
		die(`finding "${f.headline || f.problem || f.label}" has invalid severity '${f.severity}'`);
	}
	if (f.severity === 'blocking' && !(f.fix && String(f.fix).trim())) {
		die(
			`blocking finding "${f.headline || '?'}" has no fix — every blocking finding must state a fix`,
		);
	}
	if (f.confidence === 'low' && f.severity === 'blocking') {
		die(
			`finding "${f.headline || '?'}" is low-confidence but blocking — a blocker must be high-confidence (R12)`,
		);
	}
	if (f.label !== 'praise' && !(f.headline && String(f.headline).trim())) {
		die(
			`finding at ${f.file || '?'}:${f.line ?? '?'} has no headline — required for the scan table (R13)`,
		);
	}
	if (f.suggestion && String(f.suggestion).includes('\u0060\u0060\u0060')) {
		die(
			`finding "${f.headline}" has a code fence inside its suggestion — it would break the emitted block; strip the fence, the renderer adds its own`,
		);
	}
}
...
if (praiseAll.length > 2) {
	console.error(
		`render-review: note — ${praiseAll.length} praise findings; only the first 2 render (no silent caps: trim findings.json yourself if a different pair should survive)`,
	);
}
...
const sortKey = (a, b) =>
	...
	(a.line ?? 0) - (b.line ?? 0);
const ordered = [...listed].toSorted(sortKey);
...
function whereUrl(f) {
	if (!prMeta || !f.file || !prMeta.headSha) {
		return null;
	}
	...
	if (prMeta.platform === 'ado') {
		...
		if (f.line !== null) {
			u += `&line=${f.line}&lineEnd=${f.line}&lineStartColumn=1&lineEndColumn=500`;
		}
		return u;
	}
	return null;
...
const whereHtml = (f) => {
	...
	const u = whereUrl(f);
	return u ? ` · <a href="${u}"><code>${w}</code></a>` : ` · <code>${w}</code>`;
};
const bodyOf = (f) => {
...
function diffNewLines(diff) {
	...
	for (const ln of diff.split('\n')) {
		...
		if (h) {
			newNo = Number.parseInt(h[1], 10) - 1;
			continue;
		}
		if (!file) {
			continue;
		...
		if (ln.startsWith('+') && !ln.startsWith('+++')) {
			newNo++;
			byFile.get(file).set(newNo, ln.slice(1));
...
if (prMeta) {
	...
	for (const f of ordered) {
		...
		if (!changedPaths.has(f.file)) {
			die(
				`anchor gate: "${f.headline}" anchors ${f.file}, which is not a changed file of this PR — findings anchor the diff (R1); flag pre-existing code via a changed-file anchor or as a global note`,
			);
		}
		if (f.line === null) {
			continue;
		...
		if (text === undefined) {
			console.error(
				`render-review: note — ${f.file}:${f.line} ("${f.headline}") is outside the diff hunks; the diff cannot confirm its content, verify against head manually`,
			);
		} else if (f.anchor_snippet && !text.includes(f.anchor_snippet)) {
			die(
				`anchor gate: ${f.file}:${f.line} reads "${text.trim().slice(0, 80)}" — does not contain anchor_snippet "${f.anchor_snippet}" ("${f.headline}"); the line number is stale or wrong`,
			);
		}
	}
}
...
if (prMeta) {
	const scope = [];
	const nFiles =
		prMeta.filesTruncated && typeof prMeta.changedFiles === 'number'
			? prMeta.changedFiles
			: prMeta.files?.length;
	if (nFiles) {
		scope.push(`${nFiles} file${nFiles === 1 ? '' : 's'}`);
	}
	if (typeof prMeta.additions === 'number') {
		scope.push(`+${prMeta.additions} −${prMeta.deletions}`);
	}
	if (scope.length > 0) {
		headLine += ` · \`${scope.join(' · ')}\``;
	}
...
if (bottom) {
	out.push('', `> **TL;DR:** ${bottom}`);
}
if (doc.what_it_does && String(doc.what_it_does).trim()) {
	out.push('', `**Does:** ${String(doc.what_it_does).trim()}`);
}
...
if (prMeta?.ticket) {
	...
	const also =
		extras.length > 0
			? ` · also ${extras.map((x) => `[${x.custom_id || x.id}](${x.url})`).join(', ')}`
			: '';
	out.push(
		'',
		`**Ticket:** [${t.custom_id || t.id}](${t.url}) · ${t.status || 'status unknown'}${nameNote}${also}`,
	);
}
...
const changed =
	prMeta && typeof prMeta.additions === 'number' ? prMeta.additions + prMeta.deletions : null;
if (doc.coverage && String(doc.coverage).trim()) {
	...
	console.error(
		`render-review: WARNING — ${changed} changed lines exceeds ${OVERSIZE_LINES} but findings.json has no "coverage" field; R15 requires stating where the review focused on an oversized diff.`,
	);
}
...
if (verdict === 'request-changes') {
	...
	out.push(
		'',
		`**Mergeable after:** ${blockers.map((f, i) => `#${i + 1} ${String(f.headline).trim()}`).join(' · ')}`,
	);
}
...
if (ordered.length > 0) {
	if (platform === 'github') {
		...
		ordered.forEach((f, i) => {
			out.push(
				`<details><summary>${emojiOf(f)} <b>${i + 1} · ${f.label} (${f.severity})</b> — ${String(f.headline).trim()}${whereHtml(f)}</summary>`,
				'',
				bodyOf(f),
				'',
				'</details>',
		...
		ordered.forEach((f, i) => {
			...
			out.push(
				'',
				`**${i + 1}. ${emojiOf(f)} ${f.label} (${f.severity})** — ${String(f.headline).trim()}${where}`,
				'',
				bodyOf(f),
			);
		});
...
if (praise.length > 0) {
	...
	for (const p of praise) {
		out.push(`- ${String(p.problem).trim()}`);
	}
}
...
for (const re of REFUSE) {
	if (re.test(block)) {
		die(`hard-band violation: output matches ${re}`);
	}
}
for (const re of [/\bclaude\b/i, /\banthropic\b/i]) {
	if (re.test(block)) {
		console.error(
			`render-review: NOTE — output mentions ${re}; confirm it is review content, not attribution.`,
		);
	}
}
...
for (const ch of block) {
	...
	const isEmoji =
		(o >= 0x1_f3_00 && o <= 0x1_fa_ff) ||
		(o >= 0x26_00 && o <= 0x27_bf) ||
		o === 0x27_05 ||
		o === 0x1_f4_ac;
	if (isEmoji && !ALLOWED.has(ch)) {
		die(`hard-band violation: decorative emoji '${ch}' in output (only ✅ 💬 🔴 🟡 allowed)`);
	}
}
