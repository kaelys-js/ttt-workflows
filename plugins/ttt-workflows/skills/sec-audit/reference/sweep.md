...
`localStorage.setItem('...token...')`) → `candidates.jsonl (scratch dir)`. Agent layer
...
semgrep runs with `--config auto` (SFP4) — every shipped `SEC-nn` upstreams a rule
...
Prompts in `docs/deep-read-prompts/`, orchestrated by `workflows/expansion-sweep.js`.
...
drift). Output layout (SFP11): `candidates.jsonl (scratch dir)` (gitignored, regenerated),
`the coverage claim (coverage-claim.mjs)` (committed), `semgrep --config auto` (committed),
`docs/agent-triage-prompt.md` (committed), `scripts/find-findings.sh` (committed).

## Coverage claim (SFP8) — honest by construction

Every sweep ends with `the coverage claim (coverage-claim.mjs)`: "N surfaces swept, M rules applied, K raw
...
`candidates.jsonl (scratch dir)` (gitignored, regenerated), `the coverage claim (coverage-claim.mjs)`
(committed), `pocs/secXX-poc/` stubs, new `semgrep --config auto`, and the SR12 chain
