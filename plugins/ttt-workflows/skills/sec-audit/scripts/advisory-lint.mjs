...
if (!path) {
	console.error('usage: node advisory-lint.mjs <advisory.md>');
	process.exit(2);
}
...
try {
	...
	console.error(`advisory-lint: cannot read ${path}: ${error.message}`);
	process.exit(2);
}
...
for (const re of [
	/co-authored-by/i,
	/generated (with|by)\s+(claude|copilot|chatgpt|an ai|ai)/i,
	/noreply@anthropic\.com/i,
	/\u{1F916}/u,
]) {
	const m = body.match(re);
	if (m) {
		violations.push(`AI attribution: matches ${re} ("${m[0]}") — advisories carry no attribution`);
	}
}
...
for (const re of [/\bclaude\b/i, /\banthropic\b/i, /\bcopilot\b/i]) {
	...
	if (m) {
		console.error(
			`advisory-lint: NOTE — body mentions "${m[0]}"; confirm it is finding content, not a self-credit (SR11).`,
		);
	}
}
...
for (const re of [
	/github\.com\/[^\s)]+\/(issues|pull)\/\d+/i,
	/\bpublic (issue|advisory|disclosure|thread)\b/i,
]) {
	const m = body.match(re);
	if (m) {
		violations.push(
			`private-first (SR1): body references a public lane ("${m[0]}") — findings stay private until GHSA`,
		);
	}
}
...
const isStandDown =
	/\bstand[\s-]?down\b|no (new )?security (finding|vulnerabilit)|no sec-nn|severity[:\s]*none/i.test(
		body,
	);
...
const REQUIRED = isStandDown
	...
	: [
			...
			[
				/@[0-9a-f]{7,40}\b|commit[:\s@]+[0-9a-f]{7,40}\b|\b[0-9a-f]{40}\b/i,
				"a pinned commit SHA (affected component — use repo@sha, 'commit <sha>', or a full 40-char sha)",
			],
...
if (emDashes > paras) {
	console.error(
		`advisory-lint: NOTE — ${emDashes} em-dashes across ~${paras} paragraphs; ration them (SR11)`,
	);
}
for (const filler of ['genuinely', 'really', 'simply', 'just ', 'very ']) {
	const n = (body.toLowerCase().match(new RegExp(`\\b${filler.trim()}\\b`, 'g')) || []).length;
	if (n > 2) {
		console.error(`advisory-lint: NOTE — filler "${filler.trim()}" x${n} (SR11)`);
	}
}
for (const emoji of body) {
	const o = emoji.codePointAt(0);
	if ((o >= 0x1_f3_00 && o <= 0x1_fa_ff) || (o >= 0x26_00 && o <= 0x27_bf)) {
		console.error(
			`advisory-lint: NOTE — decorative emoji '${emoji}' (SR11: an advisory is not a chat message)`,
		);
		break;
	}
}
...
console.log(
	`advisory-lint: clean (attribution ✓ private-first ✓${isStandDown ? ' stand-down record — vuln-anchors waived' : ' required sections ✓'})`,
);
