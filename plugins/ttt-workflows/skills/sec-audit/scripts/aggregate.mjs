...
function layerName(fn) {
	return fn.includes('azure')
		? 'live-azure'
		: fn.includes('entra')
			? 'live-entra'
			: fn.includes('ado')
				? 'live-ado'
				: fn.includes('expansion')
					? 'expansion'
					: 'source';
}
...
for (const fn of readdirSync(dir)) {
	if (!/findings\.json$|^expansion\.json$/.test(fn)) {
		continue;
	}
	const d = loadJson(join(dir, fn));
	for (const f of d.findings || []) {
		layerFindings.push({ ...f, _file: fn });
	}
	for (const id of d.referenced_ids || []) {
		referencedIds[id] = layerName(fn);
	}
}
...
const rollup = SEV.reduce(
	(m, s) => (
		(m[s] = layerFindings.filter((f) => (f.severity || '').toUpperCase() === s).length),
		m
	),
	{},
);
console.error(
	`aggregate: ${layerFindings.length} layer findings (${SEV.map((s) => `${s[0]}${rollup[s]}`).join(' ')})${osvCount ? ` + ${osvCount} dep CVEs` : ''}`,
);

// ---- coverage matrix (only if a known list is supplied) ----
const knownPath = opt('--known');
if (!knownPath) {
	console.log(`no --known list — wrote nothing; rollup only: ${JSON.stringify(rollup)}`);
	process.exit(0);
}
if (!existsSync(knownPath)) {
	die(`--known file not found: ${knownPath}`);
}

let rows;
if (knownPath.endsWith('.json')) {
	rows = loadJson(knownPath);
} else {
	const recs = parseCsv(readFileSync(knownPath, 'utf8'));
	const hdr = recs.shift().map((h) => h.trim());
	const ci = (n) => hdr.indexOf(n);
	if (ci('id') < 0) {
		die(`--known CSV has no 'id' column (header: ${hdr.join(',')})`);
	}
	rows = recs
		.map((c) => ({
			id: (c[ci('id')] || '').trim(),
			title: c[ci('title')] || '',
			severity: c[ci('severity')] || '',
		}))
		.filter((r) => r.id);
}
...
const byId = {};
let blob = layerFindings
	.map(
		(f) =>
			`${f.id_hint || ''} ${f.title || ''} ${f.class || ''} ${f.evidence || ''} ${f.file || ''}`,
	)
	.join('\n');
// a layer may ship a free-text corpus (e.g. the deep-read's full evidence) for keyword matching
for (const fn of readdirSync(dir)) {
	if (fn.endsWith('findings.json')) {
		const d = loadJson(join(dir, fn));
		if (d.corpus) {
			blob += `\n${d.corpus}`;
		}
...
for (const f of layerFindings) {
	...
	for (const id of [f.id_hint, ...(f.ref_ids || [])]) {
		if (id && id.startsWith('SEC')) {
			(byId[id] = byId[id] || new Set()).add(L);
		}
	}
}

const cov = rows.map((r) => {
	if (remediated.has(r.id)) {
		return {
			id: r.id,
			title: r.title,
			severity: r.severity,
	...
	if (referencedIds[r.id]) {
		layers.add(referencedIds[r.id]);
	}
	const re = map[r.id];
	if (re && new RegExp(re, 'i').test(blob)) {
		layers.add('source');
	}
	return {
		id: r.id,
		title: r.title,
		severity: r.severity,
		status: layers.size > 0 ? 'found' : 'gap',
		layers: [...layers].toSorted(),
	};
});
...
console.log(
	`wrote ${out} — found ${c('found')} · remediated ${c('remediated')} · gap ${c('gap')} of ${cov.length}${
		c('gap')
			? ` · GAPS: ${cov
