...
function die(m) {
	console.error(`coverage-claim: ${m}`);
	process.exit(1);
}
const args = process.argv.slice(2);
if (args.length === 0) {
	die("usage: node coverage-claim.mjs <coverage.md> | --json '<obj>'");
}

let c;
if (args[0] === '--json') {
	try {
		c = JSON.parse(args.slice(1).join(' '));
	} catch (error) {
		die(`bad --json: ${error.message}`);
	}
} else {
	...
	const categories =
		(
			text.match(/^-\s+`(auth|secrets|transport|input|iac|cicd|storage|deps|dependencies)`:/gim) ||
			[]
		).length || null;
	c = {
		...
		untriaged_reason:
			/un[\s-]?triaged[^.]*\(([^)]*reason[^)]*)\)/i.test(text) || /need triage/i.test(text) || null,
	};
	// Format B lacks an explicit "triaged" total; derive it when both parts are known.
	if (c.triaged === null && c.confirmed !== null && c.stood_down !== null) {
		c.triaged = c.confirmed + c.stood_down;
	}
}
...
if (missing.length > 0) {
	...
	if (typeof c.hits === 'number' && typeof c.untriaged === 'number') {
		console.error(
			`coverage-claim: this report is readable but predates the SFP8 shape — has hits=${c.hits}, untriaged=${c.untriaged}, but is missing: ${missing.join(', ')}. Upgrade it to state confirmed/stood-down explicitly (SFP8).`,
		);
		process.exit(1);
	}
	die(
		`unreadable as a coverage claim — no SFP8 sentence and no recognizable bullet counts (missing: ${missing.join(', ')})`,
	);
}
...
if (c.confirmed + c.stood_down !== c.triaged) {
	die(
		`arithmetic: confirmed(${c.confirmed}) + stood_down(${c.stood_down}) != triaged(${c.triaged})`,
	);
}
...
if (c.untriaged > 0 && !c.untriaged_reason) {
	die(
		`honesty: untriaged=${c.untriaged} but no reason given (SFP8: un-triaged must carry a reason)`,
	);
}
if (c.untriaged === 0) {
	console.error(
		`coverage-claim: NOTE — untriaged=0 is unusual on a real sweep (SFP8); confirm nothing was silently dropped`,
	);
}

console.log(
	`coverage-claim: valid — ${c.surfaces} surfaces, ${c.rules} rules, ${c.hits} hits, ${c.triaged} triaged (${c.confirmed} confirmed / ${c.stood_down} stood-down), ${c.untriaged} un-triaged`,
);
