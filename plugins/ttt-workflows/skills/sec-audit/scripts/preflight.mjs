...
if (layers.includes('code')) {
	...
	for (const [cmd, why] of [
		...
		warn(
			...
			`install: pipx install ${cmd}  (or: brew install ${cmd})`,
		);
	}
	warn(
		cmdOk('gh', ['auth', 'status']),
		'gh (GitHub auth)',
		'only for GitHub repo/PR targets',
		'run:  gh auth login',
	);
}
...
if (layers.some((l) => ['azure', 'entra', 'ado'].includes(l))) {
	need(
		azOk,
		'az (Azure login)',
		'live probes: ARM (azure) + Graph (entra) + pipeline bearer (ado)',
...
if (softMissing) {
	console.log(
		`${softMissing} scanner(s)/optional auth missing (marked !): those checks are skipped and reported as NOT covered (SFP8), not silently dropped.`,
	);
}
if (missing) {
...
console.log(
	`All required tools present.${softMissing ? ' Install the ! items for full coverage.' : ''}`,
);
