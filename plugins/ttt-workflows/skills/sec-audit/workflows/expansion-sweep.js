...
export const meta = {
	...
	description:
		'Hunt NET-NEW security findings on un-deep-read surfaces and adversarially verify them',
	phases: [{ title: 'Hunt' }, { title: 'Verify' }],
};

const surfaces = (args && args.surfaces) || [];
if (surfaces.length === 0) {
	log('expansion-sweep: no surfaces in args — nothing to hunt');
	return {
		surfaces: 0,
		total_candidates: 0,
		novel_candidates: 0,
...
const SCHEMA = {
	...
	properties: {
		findings: {
			...
			items: {
				type: 'object',
				required: ['title', 'severity', 'file', 'line', 'evidence', 'why_exploitable', 'novel'],
				properties: {
					title: { type: 'string' },
...
const all = hunted
	.filter(Boolean)
	.flatMap((h) => h.findings.map((f) => ({ ...f, surface: h.surface })));
...
Read the actual file and surrounding code. A refute must prove the exploit is CLOSED (input validated, authz enforced, sink safe). "It's intended" or "needs an authed caller" is a severity caveat, NOT a refutation. Return verdict CONFIRMED / STOOD-DOWN / NEEDS-DEEPER-READ with a one-line reason.`,
				{
					...
					schema: {
						...
						properties: {
							verdict: { enum: ['CONFIRMED', 'STOOD-DOWN', 'NEEDS-DEEPER-READ'] },
							reason: { type: 'string' },
						},
					},
				},
