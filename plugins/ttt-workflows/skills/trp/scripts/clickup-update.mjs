...
// Exported so token resolution and ref parsing are unit-tested directly; the ClickUp req below
// is the only live part and is c8-ignored (real runs cover it).
...
// ---- main -------------------------------------------------------------------
// Arg parsing, the attribution + two-layer gates, ref parsing, and token resolution are all
// offline (tested here); the ClickUp req + status/comment flow needs live I/O and is c8-ignored.
async function main() {
	...
	if (!ref) {
		die(
			'usage: node clickup-update.mjs <TICKET_URL_OR_ID> [--status s] [--comment-file p] [--live]',
		);
	}
	...
	/* c8 ignore start -- live ClickUp req + status/comment flow; exercised by real runs */
	...
	const task = await req(
		'GET',
		`https://api.clickup.com/api/v2/task/${encodeURIComponent(id)}${q}`,
	);
	console.log(
	...
	if (comment) {
		...
		console.log(
			`comment posted + landed-verified (${comment.length} chars, latest comment matches)`,
		);
	}
	/* c8 ignore stop */
}
...
if (process.argv[1] && fileURLToPath(import.meta.url) === process.argv[1]) {
	try {
		...
		console.error(`clickup-update: ${error.message}`);
		process.exit(1);
	}
}
/* c8 ignore stop */
