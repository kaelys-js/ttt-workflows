...
if (typeof fetch !== "function") { console.error("clickup-update: Node 18+ required (global fetch missing)"); process.exit(1); }
...
function die(msg) { console.error(`clickup-update: ${msg}`); process.exit(1); }
...
for (let i = 0; i < args.length; i++) {
  if (args[i].startsWith("--")) {
    ...
    if (FLAGS_BARE.has(args[i])) continue;
    die(`unknown flag '${args[i]}' (known: --status <s>, --comment-file <p>, --live)`);
  }
}
const ref = args.find((a) => !a.startsWith("--"));
if (!ref) die("usage: node clickup-update.mjs <TICKET_URL_OR_ID> [--status s] [--comment-file p] [--live]");
...
if (!status && !commentFile) die("nothing to do: pass --status and/or --comment-file");
...
if (commentFile) {
  ...
  const banned = /co-authored|generated (with|by)|claude|anthropic|copilot|chatgpt|openai|\bopus\b|\bsonnet\b|\bhaiku\b|noreply@anthropic|\u{1F916}/iu;
  const m = comment.match(banned);
  if (m) die(`attribution scan FAILED: comment body matches "${m[0]}" — fix the body; nothing was posted`);
  ...
  if (!hasTech || !hasSummary) die(`two-layer check FAILED: body must contain both a "Summary (non-technical)" layer and a "Technical detail" layer (templates.md); missing: ${[!hasSummary && "summary", !hasTech && "technical"].filter(Boolean).join("+")}`);
}
...
async function req(method, url, body) {
  ...
  if (!res.ok) die(`ClickUp ${method} ${res.status}: ${txt.slice(0, 300)}`);
...
console.log(`task: ${task.custom_id || task.id} "${task.name}" · current status: ${task.status?.status}`);

if (!live) {
  console.log("--- DRY RUN (no writes; pass --live to execute) ---");
  if (status) console.log(`would set status: "${task.status?.status}" → "${status}"`);
  if (comment) console.log(`would post two-layer comment (${comment.length} chars):\n${"-".repeat(60)}\n${comment}\n${"-".repeat(60)}`);
...
if (comment) {
  await req("POST", `https://api.clickup.com/api/v2/task/${task.id}/comment`, { comment_text: comment });
  ...
  if (!latestText.startsWith(comment.slice(0, 80)))
    die(`landed-verification FAILED: latest comment is not the posted body (got: "${latestText.slice(0, 80)}…") — Phase 5 INCOMPLETE`);
