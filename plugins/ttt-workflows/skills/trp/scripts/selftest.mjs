...
function run(script, args, env = {}) {
	const r = spawnSync('node', [join(D, script), ...args], {
		encoding: 'utf8',
		env: { ...process.env, ...env },
	});
...
for (const [name, ref, expectIn] of [
	...
	const r = run('fetch-ticket.mjs', [ref, '--out', join(tmp, 't.json')], {
		...ENV,
		CLICKUP_TEAM_ID: '1',
	});
	// dummy token → ClickUp 401; the error URL proves the ref parsed to the right endpoint
	check(
		`fetch-ticket parses ${name}`,
		r.code !== 0 && r.err.includes(expectIn),
		r.err.trim().slice(0, 100),
	);
}
...
check(
	'update refuses attribution (pre-network)',
	r.code !== 0 && /attribution scan FAILED/.test(r.err) && !/ClickUp/.test(r.err),
	r.err.trim().slice(0, 100),
);

r = run('clickup-update.mjs', ['PROJ-1', '--comment-file', single], ENV);
check(
	'update refuses single-layer (pre-network)',
	r.code !== 0 && /two-layer check FAILED/.test(r.err),
	r.err.trim().slice(0, 100),
);
...
check(
	'dry-run reaches only the resolve GET (401, no write)',
	r.code !== 0 && /ClickUp GET 401/.test(r.err),
	r.err.trim().slice(0, 100),
);
...
const firstPut = src.search(/req\(['"]PUT/);
const firstPost = src.search(/req\(['"]POST/);
check(
	'write verbs only after the dry-run exit',
	dryExit > 0 && firstPut > dryExit && firstPost > dryExit,
);

// attribution regex parity with gates.md (the mechanical scan and the doc agree)
check(
	'attribution scan covers model names + robot emoji',
	/opus|sonnet|haiku/.test(src) && src.includes('1F916'),
);

// unknown-flag refusals (typo safety: --liv must NOT silently dry-run)
r = run('clickup-update.mjs', ['PROJ-1', '--comment-file', good, '--liv'], ENV);
check(
	"update refuses typo'd --liv",
	r.code !== 0 && /unknown flag/.test(r.err),
	r.err.trim().slice(0, 90),
);
r = run('fetch-ticket.mjs', ['PROJ-1', '--otu', 'x.json'], ENV);
check(
	"fetch-ticket refuses typo'd flag",
	r.code !== 0 && /unknown flag/.test(r.err),
	r.err.trim().slice(0, 90),
);
...
check(
	'update refuses --status with missing value',
	r.code !== 0 && /needs a value/.test(r.err),
	r.err.trim().slice(0, 90),
);
r = run('fetch-ticket.mjs', ['PROJ-1', '--out'], ENV);
check(
	'fetch-ticket refuses --out with missing value',
	r.code !== 0 && /needs a value/.test(r.err),
	r.err.trim().slice(0, 90),
);
r = run(
	'clickup-update.mjs',
	['PROJ-1', '--comment-file', join(tmp, 'cop.md')],
	ENV,
	writeFileSync(
		join(tmp, 'cop.md'),
...
check(
	'update refuses other-vendor attribution (copilot)',
	r.code !== 0 && /attribution scan FAILED/.test(r.err),
	r.err.trim().slice(0, 90),
);
